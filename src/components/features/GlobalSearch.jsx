  ...
  useEffect(()=>{
    ...
    const fetchData = async(query)=>{
      try{
        ...
        const res = await axios.get(`${import.meta.env.VITE_FETCH_URL}/api/v1/search-suggest?q=${query}`);
        console.log("this is the data coming",res);
            ...
            <ul className="">
              {result.map((item , idx) => (
                ...
                >
                  ...
                  <p className="text-xs text-gray-400">{item.category} — {item.location.address}</p>
                </li>
              ))}
            </ul>
          </div>
