...
const EventDetail = () => {
  ...
  const { data, error, loading } = useFetch(`${import.meta.env.VITE_FETCH_URL}/api/events/${slug}`);

  useEffect(() => {
    document.title = `CityScouts | Event Details`;
  }, []);
  ...
  // Prepare gallery images (future-proof for multiple images)
  const galleryImages =
    data.galleryImages && data.galleryImages.length > 0 ? data.galleryImages : [data.mainImage];

  // Extract coordinates for MapView
      ...
      <div className="md:max-w-[85vw] max-w-[97vw] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0">
            {/* Gallery and Quick Facts Side by Side */}
